...
image = hs.load(os.path.join(path_to_data, "filtered HAADF STEM PTO 100 0030.tif"))
sampling = 0.01655927973 #  nm/pix
units = 'nm'
...
atom_lattice = am.load_atom_lattice_from_hdf5(
    os.path.join(path_to_data, "Atom_Lattice.hdf5"), False)
...
customisability.
'''
line_profile_positions = np.load(os.path.join(path_to_data, 'line_profile_positions.npy'))
...
compare_images_line_profile_one_image(strain_map, line_profile_positions,
                            linewidth=100, arrow='h', linetrace=0.05, **kwargs)
...
rotation_map = tmlp.rotation_of_atom_planes(sublattice1, zone_vector_index_A,
...
ca_ratio_map = tmlp.ratio_of_lattice_spacings(sublattice1, zone_vector_index_B,
...
''' Polarisation needs to be done with old method, atomap doesn't work it seems'''
...
tmlp.plot_polarisation_vectors(x=x, y=y, u=u, v=v, image=image.data,
